...
const Geographics: React.FC<GeographicsProps> = ({ states }) => {
	...
	return (
		<div className="bg-white rounded-[10px] h-[600px] w-full max-w-[600px] ">
			...
			<div className="w-full bg-clearblue text-center mb-[30px] text-eccblue border-y border-dashed flex justify-center items-center  border-y-eccblue h-[45px]">
				<p> Total percentage of users per state</p>
			</div>
			<div className="grid grid-cols-2 gap-y-[30px] ">
				{topStates.map((state) => (
					<div
						key={state.name}
						className="px-4 gap-y-6"
					>
						...
						<div className="h-[3px] rounded-md w-full bg-[#d3d3d3]">
							<div
								style={{ width: `${getPercentage(state.users)}%` }}
								className={`h-full rounded-md bg-eccblue  `}
							></div>
						</div>
					</div>
				))}
